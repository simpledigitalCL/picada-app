...
# Photo migration: proxy/Google URLs → Supabase Storage (bucket `place-photos`)
# Without args: migrates all places with proxy/Google URLs in `gallery`.
# With external_id/place_id: migrates only that place (useful for testing).
...
| `lib/server/photo-storage.ts` | Helpers to upload Google photos to Supabase Storage bucket `place-photos` |
...
When a place is inserted as `active` or transitions to `active`, the Postgres trigger `trg_invalidate_discovery_cache_on_active_place` (migration `20260531_invalidate_discovery_cache_on_active_place.sql`) deletes all `daily::%` rows from `place_discovery_cache`. Without this, newly active places stay invisible in discover/map/review-search for up to 26h because the daily snapshot is read **before** the `places` table (see Discover API performance architecture below). The trigger is broad — it nukes all daily snapshots, not just the matching zone. Acceptable at current scale; revisit when traffic grows.
