...
export function AuthQuickRegister() {
  ...
  const handleOAuth = async (provider: 'google' | 'facebook') => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        // Vuelve a la SPA; detectSessionInUrl detecta la sesión al cargar.
        redirectTo: window.location.origin,
        // Permite elegir cuenta de Google en vez de reusar la última.
        ...(provider === 'google' ? { queryParams: { prompt: 'select_account' } } : {}),
      },
    })
    // En éxito el navegador redirige fuera; sólo llegamos aquí si falla.
    if (error) {
      setStatus({ msg: `No se pudo conectar con ${provider === 'google' ? 'Google' : 'Facebook'}. Intenta de nuevo.`, error: true })
